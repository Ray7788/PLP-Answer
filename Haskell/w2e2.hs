...
-- Similarly, given two types we can
-- construct the type of functions from one to
-- the other
...
-- We can use more than one type variable
-- to indicate which inputs have to have
-- matching types (Generic type)
myApp :: (a -> b) -> a -> b
myApp f x = f x
